# AGENTS.md — Moshaheb Hossain Portfolio

Personal portfolio for **Md. Moshaheb Hossain** (Business Analyst & Product Owner). Built with TanStack Start on Netlify. Dark editorial aesthetic with Cormorant Garamond + DM Sans typography and a warm gold (#C9A55A) accent palette.

## Routes

| File | URL | Purpose |
|------|-----|---------|
| `index.tsx` | `/` | Hero, stats, competencies, CTA |
| `about.tsx` | `/about` | Photo gallery, bio, tools, BA artefacts |
| `projects.tsx` | `/projects` | 8 domain expertise cards (inline data) |
| `resume.tsx` | `/resume` | Timeline experience + education + certs |
| `contact.tsx` | `/contact` | Social links + Netlify Forms |
| `blog/$slug.tsx` | `/blog/*` | Template remnant — not in nav, leave alone |

## Design System

**Palette** (always dark — no light mode toggle):
- Background: `#0A0908`, Surface: `#0D0C0A`, Gold: `#C9A55A`
- Text: `#F0EBE3`, Muted: `#7A7166`, Border: `#1C1914`

**Fonts**: Cormorant Garamond (headings) + DM Sans (body) loaded via Google Fonts in `__root.tsx` head links.

**CSS classes** (all in `src/styles.css`):
`.btn-gold`, `.btn-outline-gold`, `.tag-pill`, `.section-label`, `.nav-link(.active)`, `.card-lift`, `.gallery-img`, `.timeline-line/.timeline-dot`, `.form-input`, `.anim-fade-up/in/slide-r` + `.delay-100…600`

## Images

All images use Netlify Image CDN: `/.netlify/images?url=%2Fphoto.jpg&w=600&h=400&fit=cover`. URI-encode the `url` param. Source files: `public/profile.jpg`, `public/photo-1/2/3.jpg`.

## Content

Update CV data in `content/jobs/*.md` and `content/education/*.md`. Schema fields: `jobTitle`, `company`, `location`, `startDate` (ISO date), `endDate?`, `summary`, `tags[]`. Markdown body → `job.content`.

The `projects.tsx` domain cards use **inline data** (a `DOMAINS` array) — no content-collections dependency.

## Project Overview

Personal portfolio for Md. Moshaheb Hossain. Built with TanStack Start and deployed on Netlify.
